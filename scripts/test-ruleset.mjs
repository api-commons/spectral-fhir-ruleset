#!/usr/bin/env node
// Test harness for @api-common/spectral-fhir-ruleset.
//
...
//
// The clean fixture is a small but genuinely FHIR-shaped API, so assertion 2
// doubles as a guard against the resource-type and scope patterns over-matching.
//
...
const rulesetPath = resolve(root, 'fhir.yaml');
...
function lint(fixture) {
  ...
  const outFile = resolve(tmpdir(), `fhir-spectral-${fixture}-${process.pid}.json`);
  const res = spawnSync(
    bin,
...
if (clean.length) {
  ...
  console.log('   (Which also proves the resource-type and SMART-scope patterns do');
  console.log('    not over-match a correctly built FHIR API.)');
}
